...
$sql = "SELECT (SELECT count(id) from agendamentos where usuario = ? and date(reg_agendamentos) between '$data_inicio' and '$data_fim') as quant_agendamento, (SELECT count(id) from vendas where usuario = ? and date(reg_venda) between '$data_inicio' and '$data_fim') as quant_venda,(SELECT count(id) from controle where usuario = ? and  visita between '$data_inicio' and '$data_fim') as quant_visita";
